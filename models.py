...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        self.cache_embedding(out)
        out = self.linear(out)
        return out

    def cache_embedding(self, x):
        self.embedding = x
...
def log_reg(weight_decay):
    return LogisticRegression(weight_decay)
...
class ConvNet(nn.Module):
    def __init__(self):
        ...
        self.linear = nn.Linear(4 * 4 * 100, 10)
...
class ConvNetSmall(nn.Module):
    def __init__(self):
        ...
        # fully connected layer, output 10 classes
        self.linear = nn.Linear(4 * 4 * 25, 10)
...
def fc_net(input_size, output_size):
    return FCNet(input_size, output_size)
